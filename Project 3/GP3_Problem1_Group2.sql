...
We chose the table 'movie' because it is the table that is accessed most
among the list of queries and because it contains many attributes.
The attribute we chose to index was genre becausethe  primary key, mname,
is already indexed, and genre was the next-to-most occurring attribute
in WHERE statements. The index is a secondary index because mname is
already a primary index due to being a primary key and because it is not
sorted according to genre. The reason we chose these queries to run was
because these queries are comparing the genre of one Movie to another.
*/